...
C_API long (*get_orig_syscall(void))(long number, ...) {
    if (!orig_syscall) {
        orig_syscall = reinterpret_cast<long (*)(long number,
                                                 ...)>(FXHandler::FindLibcSymbolRealAddress(
                "syscall"));
        CHECK(orig_syscall);
    }
    return orig_syscall;
...
INTERCEPT_SYSCALL(openat, int fd, const char *pathname, int flags, int mode) {
    ...
    return orig_syscall(__NR_openat, fd, IoRedirect::GetRedirect(pathname),
                        force_O_LARGEFILE(flags), mode);
}
...
INTERCEPT_SYSCALL(fstatat, int dir_fd, const char *path, struct stat *buf, int flags) {
    LOG("syscall fd: %d, path: %s, flags: %d", dir_fd, path, flags);
    IS_BLACKLIST_FILE(path);
...
INTERCEPT_SYSCALL(faccessat, int fd, const char *path, int mode) {
    LOG("syscall fd: %d, path: %s, mode: %d", fd, path, mode);
    IS_BLACKLIST_FILE(path);
...
INTERCEPT_SYSCALL(mkdirat, int dir_fd, const char *path, mode_t mode) {
    LOG("syscall fd: %d, path: %s, mode: %d", dir_fd, path, mode);
    IS_BLACKLIST_FILE(path);
...
INTERCEPT_SYSCALL(readlinkat, int dir_fd, const char *path, char *buf, size_t buf_size) {
    LOG("syscall fd: %d, path: %s", dir_fd, path);
    IS_BLACKLIST_FILE(path);
    const char *redirect = IoRedirect::GetRedirect(path);
...
INTERCEPT_SYSCALL(symlinkat, const char *old_path, int new_dir_fd, const char *new_path) {
    LOG("syscall old path: %s, new fd: %d, new path: %s", old_path, new_dir_fd, new_path);
    IS_BLACKLIST_FILE(old_path);
    return orig_syscall(number, IoRedirect::GetRedirect(old_path), new_dir_fd,
                        IoRedirect::GetRedirect(new_path));
}

INTERCEPT_SYSCALL(unlinkat, int dirfd, const char *path, int flags) {
    LOG("syscall fd: %d, path: %s, flags: %d", dirfd, path, flags);
    IS_BLACKLIST_FILE(path);
    return orig_syscall(number, dirfd, IoRedirect::GetRedirect(path), flags);
}

INTERCEPT_SYSCALL(utimensat, int dir_fd, const char *path, const struct timespec times[2],
                  int flags) {
    LOG("syscall fd: %d, path: %s, flags: %d", dir_fd, path, flags);
    IS_BLACKLIST_FILE(path);
...
INTERCEPT_SYSCALL(truncate, const char *path, off_t length) {
    LOG("syscall path: %s, length: %ld", path, length);
    IS_BLACKLIST_FILE(path);
...
INTERCEPT_SYSCALL(getcwd, char *buf, size_t size) {
    LOG("syscall getcwd size: %zd", size);
...
C_API long syscall(long number, ...) {
    ...
    switch (number) {
#ifndef __aarch64__
        case __NR_open:
            ret = CALL_INTERCEPT(openat, AT_FDCWD, reinterpret_cast<const char *>(arg[0]),
                                 GPOINTER_TO_SIZE(arg[1]), GPOINTER_TO_SIZE(arg[2]));
            break;
#endif
        case __NR_openat:
            ret = CALL_INTERCEPT(openat, GPOINTER_TO_SIZE(arg[0]),
                                 reinterpret_cast<const char *>(arg[1]), GPOINTER_TO_SIZE(arg[2]),
                                 GPOINTER_TO_SIZE(arg[3]));
            break;
        case __NR_faccessat:
            ret = CALL_INTERCEPT(faccessat, GPOINTER_TO_SIZE(arg[0]),
                                 reinterpret_cast<const char *>(arg[1]), GPOINTER_TO_SIZE(arg[2]));
            break;
        case __NR_fchmodat:
            ret = CALL_INTERCEPT(fchmodat, GPOINTER_TO_SIZE(arg[0]),
                                 reinterpret_cast<const char *>(arg[1]), GPOINTER_TO_SIZE(arg[2]));
            break;
        case __NR_fchownat:
            ret = CALL_INTERCEPT(fchownat, GPOINTER_TO_SIZE(arg[0]),
                                 reinterpret_cast<const char *>(arg[1]), GPOINTER_TO_SIZE(arg[2]),
                                 GPOINTER_TO_SIZE(arg[3]),
        ...
        case __NR_newfstatat:
            ...
            case __NR_fstatat64:
#endif
            ret = CALL_INTERCEPT(fstatat, GPOINTER_TO_SIZE(arg[0]),
                                 reinterpret_cast<const char *>(arg[1]),
                                 reinterpret_cast<struct stat *>(arg[2]), GPOINTER_TO_SIZE(arg[3]));
            break;
        case __NR_linkat:
            ret = CALL_INTERCEPT(linkat, GPOINTER_TO_SIZE(arg[0]),
                                 reinterpret_cast<const char *>(arg[1]), GPOINTER_TO_SIZE(arg[2]),
                                 reinterpret_cast<const char *>(arg[3]),
                                 GPOINTER_TO_SIZE(arg[4]));
            break;
        case __NR_mkdirat:
            ret = CALL_INTERCEPT(mkdirat, GPOINTER_TO_SIZE(arg[0]),
                                 reinterpret_cast<const char *>(arg[1]), GPOINTER_TO_SIZE(arg[2]));
            break;
        case __NR_mknodat:
            ret = CALL_INTERCEPT(mknodat, GPOINTER_TO_SIZE(arg[0]),
                                 reinterpret_cast<const char *>(arg[1]), GPOINTER_TO_SIZE(arg[2]),
                                 GPOINTER_TO_SIZE(arg[3]));
            break;
        case __NR_readlinkat:
            ret = CALL_INTERCEPT(readlinkat, GPOINTER_TO_SIZE(arg[0]),
                                 reinterpret_cast<const char *>(arg[1]),
                                 reinterpret_cast<char *>(arg[2]), GPOINTER_TO_SIZE(arg[3]));
            break;
        case __NR_renameat:
        case __NR_renameat2:
            ret = CALL_INTERCEPT(renameat, GPOINTER_TO_SIZE(arg[0]),
                                 reinterpret_cast<const char *>(arg[1]), GPOINTER_TO_SIZE(arg[2]),
                                 reinterpret_cast<const char *>(arg[3]),
                                 GPOINTER_TO_SIZE(arg[4]));
            break;
        case __NR_symlinkat:
            ret = CALL_INTERCEPT(symlinkat, reinterpret_cast<const char *>(arg[0]),
                                 GPOINTER_TO_SIZE(arg[1]), reinterpret_cast<const char *>(arg[2]));
            break;
        case __NR_unlinkat:
            ret = CALL_INTERCEPT(unlinkat, GPOINTER_TO_SIZE(arg[0]),
                                 reinterpret_cast<const char *>(arg[1]), GPOINTER_TO_SIZE(arg[2]));
            break;
        case __NR_utimensat:
            ret = CALL_INTERCEPT(utimensat, GPOINTER_TO_SIZE(arg[0]),
                                 reinterpret_cast<const char *>(arg[1]),
                                 reinterpret_cast<const timespec *>(arg[2]),
                                 GPOINTER_TO_SIZE(arg[3]));
            break;
#if defined(__LP64__)
        case __NR_truncate:
            ret = CALL_INTERCEPT(truncate, reinterpret_cast<const char *>(arg[0]),
                                 GPOINTER_TO_SIZE(arg[1]));
            break;
#else
            case __NR_truncate:
...
#if defined(__LP64__)
        case __NR_statfs:
            ret = CALL_INTERCEPT(statfs, reinterpret_cast<const char *>(arg[0]),
                                 reinterpret_cast<struct statfs *>(arg[1]));
            break;
#else
            case __NR_statfs64:
        ...
        case __NR_chdir:
            ...
            break;
        case __NR_getcwd:
            ret = CALL_INTERCEPT(getcwd, reinterpret_cast<char *>(arg[0]),
                                 GPOINTER_TO_SIZE(arg[1]));
            break;
        default:
