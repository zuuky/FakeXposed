...
FUN_INTERCEPT API_PUBLIC void *dlsym(void *handle, const char *symbol) {
    ...
    if (handle == FXHandler::Get()->libc_handle || handle == FXHandler::Get()->libc_soinfo) {
        int error_code;
        result = remote->CallSoinfoFunction(kSFCallDlsym, kSPOriginal,
                                            FXHandler::Get()->self_soinfo, kSPSymbol, symbol,
                                            &error_code);
...
android_dlopen_ext(const char *filename, int flag, const android_dlextinfo *extinfo) {
    ...
    if (extinfo != nullptr && extinfo->library_namespace != nullptr) {
        remote->CallNamespaceFunction(kNFAddGlobalSoinfoToNamespace, kNPOriginal,
                                      extinfo->library_namespace, kNPAddress, nullptr, &error_code);
    }
...
FUN_INTERCEPT API_PUBLIC void *
android_create_namespace(const char *name, const char *ld_library_path,
    ...
    return remote->CallCreateNamespaceImpl(name, IoRedirect::GetRedirectDirectory(ld_library_path),
                                           IoRedirect::GetRedirectDirectory(default_library_path),
                                           type,
                                           IoRedirect::GetRedirectDirectory(
